...
class shm_daq_files:
    ...
    def save(self, DAQ_data):
        # if len(DAQ_data) > 0:
        #     print('size={}'.format(len(DAQ_data[0])))
        for datum in DAQ_data:
            time_ = floor(datum[0] / (self.timeLength)) * self.timeLength
            ...
            if len(self.datafileName) == 0:
                ...
                self.datafileName = datafileName_
                self.dataFile = open(self.datafilePath + self.datafileName, "ba")
            data4file = array_.array('d', datum)
