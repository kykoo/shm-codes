...
void ADXL362::begin(int16_t chipSelectPin) {
	...
	// soft reset
	//Serial.println("0, before SPIwriteOneRegister");
	//Serial.flush();
	...
	//Serial.println("0, before SPIwriteOneRegister");
	//Serial.flush();
	SPIwriteOneRegister(0x2C, 0x12);  // Write to FILTER_CONTROL: 2g, LPS=1/4*ODR, 50Hz
	//Serial.println("0, before SPIwriteOneRegister");
	//Serial.flush();
	SPIwriteOneRegister(0x2D, 0x20);  // Write to POWER_CONTROL: Ultralow noise mode
	// ==== KK'S CHANGE ENDS ====
