...
void ADXL362::begin(int16_t chipSelectPin) {
	...
	//Serial.println("0, before spi.begin");
	//Serial.flush();
	SPI.begin();
	//Serial.println("0, before setDataMode");
	//Serial.flush();
	SPI.setDataMode(SPI_MODE0);	//CPHA = CPOL = 0    MODE = 0
	//Serial.println("0, before setClockDivier");
	//Serial.flush();
	...
	// Write to FILTER_CONTROL: 2g, LPS=1/4*ODR, 50Hz
	// SPIwriteOneRegister(0x2C, 0x12);  
	
	// Write to FILTER_CONTROL: 8g, LPS=1/2*ODR, 100Hz
	//SPIwriteOneRegister(0x2C, 0b10000011);

	// Write to FILTER_CONTROL: 8g, LPS=1/2*ODR, 200Hz
#ifdef ADXL362_DEBUG
	Serial.println("0, Filter Control\n");
    Serial.flush();
   	delay(1000);
#endif
	//SPIwriteOneRegister(0x2C, 0b10000100);
	
	// Write to FILTER_CONTROL: 8g, LPS=1/2*ODR, 400Hz
	SPIwriteOneRegister(0x2C, 0b10000101);
	
	// Write to POWER_CONTROL: Ultralow noise mode
#ifdef ADXL362_DEBUG
	Serial.println("0, Ultra Low noise\n");
    Serial.flush();
    delay(1000);
#endif
	SPIwriteOneRegister(0x2D, 0x20);  
...
int8_t ADXL362::dataReadyTimeout(){
  uint16_t Timeout = 33333;              // Corresponds to 100 milli-sec
  uint16_t timer_count = 0;             // Timer count for MISO
  while (1){
    int8_t status = SPIreadOneRegister(0x0B);
    if(bitRead(status,0)) break;
    if (timer_count++ > Timeout)     // If timeout, return 1 (failure)
    {
      return (1);
    }
    else
      // delayMicroseconds(1);
      __asm__ __volatile__ ("nop"); // delay 1 clock-cycle
